...
bool bufdatabegin(struct wlr_buffer *wlr_buffer, uint32_t flags,
                  void **data, uint32_t *format, size_t *stride) {
  Buffer *buf = wl_container_of(wlr_buffer, buf, base);
...
void closemon(Monitor *m) {
  ...
  wl_list_for_each(c, &clients, link) {
    if (c->isfloating && c->geom.x > m->m.width)
      resize(c, (struct wlr_box){.x = c->geom.x - m->w.width, .y = c->geom.y, .width = c->geom.width, .height = c->geom.height}, 0, 1);
    if (c->mon == m)
...
void keypressglobal(struct wlr_surface *last_surface, struct wlr_keyboard *keyboard, struct wlr_keyboard_key_event *event, uint32_t mods, xkb_keysym_t keysym) {
...
void motionnotify(uint32_t time, struct wlr_input_device *device, double dx, double dy,
  ...
  if (cursor_mode == CurMove) {
    ...
    resize(grabc, (struct wlr_box){.x = (int)round(cursor->x) - grabcx, .y = (int)round(cursor->y) - grabcy, .width = grabc->geom.width, .height = grabc->geom.height}, 1, 1);
    return;
  } else if (cursor_mode == CurResize) {
    resize(grabc, (struct wlr_box){.x = grabc->geom.x, .y = grabc->geom.y, .width = (int)round(cursor->x) - grabc->geom.x, .height = (int)round(cursor->y) - grabc->geom.y}, 1, 1);
    return;
  }
...
void setup(void) {
  ...
  if (!hide_source)
    die("failed to create cursor hide timer");
...
void tile(Monitor *m) {
  ...
  wl_list_for_each(c, &clients, link) {
    if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
      continue;
    if (i < m->nmaster) {
      int h = (wa.height - my - ih * (MIN(n, m->nmaster) - i - 1)) / (MIN(n, m->nmaster) - i);
      resize(c, (struct wlr_box){.x = wa.x, .y = wa.y + my, .width = mw, .height = h}, 0, draw_borders);
      my += h + ih;
    } else {
      ...
      resize(c, (struct wlr_box){.x = sx, .y = wa.y + ty, .width = sw, .height = h}, 0, draw_borders);
      ty += h + ih;
    }
    i++;
  }
...
void configurex11(struct wl_listener *listener, void *data) {
  ...
  if ((c->isfloating && c != grabc) || !c->mon->lt[c->mon->sellt]->arrange) {
    resize(c, (struct wlr_box){.x = event->x - c->bw, .y = event->y - c->bw, .width = event->width + c->bw * 2, .height = event->height + c->bw * 2}, 0, 1);
  } else {
    arrange(c->mon);
  }
}
