...
- There are two builds from the same source: the running compositor binary (`dwl`) and a shared library (`dwl.so`). The binary is tiny glue code that uses `dlopen` to load `dwl.so` at startup and calls into the functions inside it.
- Every Wayland event hook in dwl is a `wl_listener` struct whose `notify` field points to a function. When a listener is registered, a helper macro stores a pointer to it in a global linked list called `listeners`.
- Hitting the reload binding calls `reload()`, which simply tries to `dlopen` a fresh `dwl.so` (searched in the paths listed above). No process restart happens—the existing Wayland connection and windows stay alive.
- If loading succeeds, `reload()` walks the `listeners` list, and for each listener it uses `dladdr` to read the current function name and `dlsym` to look up the function with the same name in the new `dwl.so`. It then swaps the listener’s `notify` pointer to the new code, effectively “hot-swapping” the callbacks.
- Finally the old module handle is dropped, the new one is stored, and a `notify-send` toast reports success; failures show an error toast instead. Because only function pointers change, compositor state (tags, layout, windows) is preserved across reloads.
